...
export function planWrapperReleaseOperations(input) {
  const release = requireObject(input, "wrapper release input")
  assertReleasePreflight({ ...release, kind: "wrapper" })
  if (release.runtimeVerified !== true) {
    throw new Error("configured public runtime must be verified before npm publication")
  }
  ...
  return release.npmIntegrity === null
    ? [`publish-npm:${release.tarball.name}`]
    : ["verify-npm-integrity"]
}

export function planRuntimeReleaseOperations(input) {
  const release = requireObject(input, "runtime release input")
  assertReleasePreflight({ ...release, kind: "runtime" })
  const assets = requireAssets(release.assets)
  const operations = []
  const existing = release.release
  if (existing === null) {
    operations.push("create-draft-release")
  } else {
    requireObject(existing, "existing release")
...
function requireAssets(value) {
  if (!Array.isArray(value) || value.length === 0) throw new TypeError("assets must be non-empty")
...
function assertTagVersion(tag, version, kind) {
  if (typeof version !== "string" || !/^\d+\.\d+\.\d+$/.test(version)) {
    throw new TypeError("version must be strict SemVer")
  }
  const expected = kind === "wrapper" ? `v${version}` : `runtime-v${version}`
  if (tag !== expected) throw new Error(`tag must equal ${expected}`)
}

function tagKind(tag) {
  if (typeof tag !== "string") return undefined
  if (/^v\d+\.\d+\.\d+$/.test(tag)) return "wrapper"
  if (/^runtime-v\d+\.\d+\.\d+$/.test(tag)) return "runtime"
  return undefined
}

function tagPrefix(kind) {
  return kind === "wrapper" ? "refs/tags/v*" : "refs/tags/runtime-v*"
}
...
async function main(args) {
  ...
  const tag = args[2]
  if (!findActiveTagRuleset(rulesets, tag)) {
    const kind = tagKind(tag)
    if (kind === undefined) throw new Error("tag must be a v<SemVer> or runtime-v<SemVer> release tag")
    throw new Error(`active tag ruleset must protect ${tagPrefix(kind)}`)
  }
